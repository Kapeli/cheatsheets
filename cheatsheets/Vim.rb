cheatsheet do
    ...
    keyword 'vim'
    source_url 'http://www.fprintf.net/vimCheatSheet.html'
    ...
    category do
        ...
        entry do
            command ':q[uit]'
            name 'Quit Vim.  This fails when changes have been made.'
        end
        entry do
            command ':q[uit]!'
        ...
        entry do
            command ':[range]wq[!]'
            name '[file]  Same as above, but only write the lines in [range].'
        end
        entry do
            command 'ZZ'
        ...
        entry do
            command 'ZQ'
            name 'Quit current file and exit (same as ":q!").'
        end
    ...
    category do
        ...
        entry do
            command ':e[dit]'
            name 'Edit the current file.  This is useful to re-edit the current file, when it has been changed outside of Vim.'
        end
        entry do
            command ':e[dit]!'
            name 'Edit the current file always.  Discard any changes to the current buffer.  This is useful if you want to start all over again.'
        end
        entry do
            command ':e[dit] {file}'
            name 'Edit {file}.'
        ...
        entry do
            command ':e[dit]! {file}'
            name 'Edit {file} always.  Discard any changes to the current buffer.'
        end
        entry do
            command 'gf'
            name 'Edit the file whose name is under or after the cursor.  Mnemonic: "goto file".'
        end
    end
    ...
    category do
        ...
        entry do
            command '{Visual}x  or {Visual}d'
            name 'Delete the highlighted text (for {Visual} see Selecting Text).'
        end
        entry do
            command '{Visual}CTRL-H   or {Visual}<BS>'
            name 'When in Select mode: Delete the highlighted text'
        end
        entry do
            command '{Visual}X  or {Visual}D'
        ...
        entry do
            ...
            name 'Delete [range] lines (default: current line)'
        end
        entry do
    ...
    category do
        ...
        entry do
            command '~'
            name 'Switch case of the character under the cursor and move the cursor to the right.  If a [count] is given, do that many characters.'
        end
        entry do
            command '~{motion}'
    ...
    category do
        id 'Substituting'
        entry do
            command ':[range]s[ubstitute]/{pattern}/{string}/[c][e][g][p][r][i][I] [count]'
            name 'For each line in [range] replace a match of {pattern} with {string}.'
        end
        entry do
            command ':[range]s[ubstitute] [c][e][g][r][i][I] [count] :[range]&[c][e][g][r][i][I] [count]'
            name 'Repeat last :substitute with same search pattern and substitute string, but without the same flags.  You may add extra flags'
        end
        entry do
            command 'notes'
            name ''
    ...
    category do
        ...
        entry do
            command '"{a-zA-Z0-9.%#:-"}'
            name 'Use register {a-zA-Z0-9.%#:-"} for next delete, yank or put (use uppercase character to append with delete and yank) ({.%#:} only work with put).'
        end
        entry do
            command ':reg[isters]'
        ...
        entry do
            command '["x]Y'
            name 'yank [count] lines \[into register x\] (synonym for yy).'
        end
        entry do
            command '{Visual}["x]y'
            name 'Yank the highlighted text \[into register x\] (for {Visual} see Selecting Text).'
        end
        entry do
            command '{Visual}["x]Y'
    ...
    category do
        ...
        entry do
            command 'l or <Right> or <Space>'
            name '[count] characters to the right (exclusive).'
        end
        entry do
            command 'k  or <Up> or CTRL-P'
            name '[count] lines upward'
        end
        entry do
            command 'j or <Down> or CTRL-J or <NL> or CTRL-N'
        ...
        entry do
            command 'g0 or g<Home>'
            name 'When lines wrap (\'wrap on): To the first character of the screen line (exclusive).  Differs from "0" when a line is wider than the screen.  When lines don\'t wrap (\'wrap\' off): To the leftmost character of the current line that is on the screen.  Differs from "0" when the first character of the line is not on the screen.'
        end
        entry do
            command 'g^'
            name 'When lines wrap (\'wrap\' on): To the first non-blank character of the screen line (exclusive).  Differs from "^" when a line is wider than the screen.  When lines don\'t wrap (\'wrap\' off): To the leftmost non-blank character of the current line that is on the screen.  Differs from "^" when the first non-blank character of the line is not on the screen.'
        end
        entry do
            command 'g$ or g<End>'
            name 'When lines wrap (\'wrap\' on): To the last character of the screen line and [count - 1] screen lines downward (inclusive).  Differs from "$" when a line is wider than the screen.  When lines don\'t wrap (\'wrap\' off): To the rightmost character of the current line that is visible on the screen.  Differs from "$" when the last character of the line is not on the screen or when a count is used.'
        end
        entry do
            command 'f{char}'
        ...
        entry do
            command '-  <minus>'
            name '[count] lines upward, on the first non-blank character
            (linewise).'
        end
        entry do
        ...
        entry do
            command ']]'
            name '[count] sections forward or to the next \'{\' in the first column.  When used after an operator, then the \'}\' in the first column.'
        end
        entry do
            command ']['
    ...
    category do
        ...
        entry do
            command 'm{a-zA-Z}'
            name 'Set mark {a-zA-Z} at cursor position (does not move the cursor, this is not a motion command).'
        end
        entry do
            command 'm\'  or m\`'
            name 'Set the previous context mark.  This can be jumped to with the "''" or "``" command (does not move the cursor, this is not a motion command).'
        end
        entry do
            command ':[range]ma[rk] {a-zA-Z}'
            name 'Set mark {a-zA-Z} at last line number in [range], column 0.  Default is cursor line.'
        end
        entry do
    ...
    category do
        ...
        entry do
            command '//{offset}<CR>'
            name 'Search forward for the [count]\'th latest used pattern with new.  If {offset} is empty no offset is used.'
        end
        entry do
            command '?{pattern}[?]<CR>'
        ...
        entry do
            command '??{offset}<CR>'
            name 'Search backward for the [count]\'th latest used pattern with new {offset}.  If {offset} is empty no offset is used.'
        end
        entry do
            command 'n'
    ...
    category do
        ...
        entry do
            command 'CTRL-Z'
            name 'Suspend Vim, like ":stop".  Works in Normal and in Visual mode.  In Insert and Command-line mode, the CTRL-Z is inserted as a normal character.'
        end
        entry do
            command ':sus[pend][!]  or :st[op][!]'
            name 'Suspend Vim.  If the \'!\' is not given and \'autowrite\' is set, every buffer with changes and a file name is written out.  If the \'!\' is given or \'autowrite\' is not set, changed buffers are not written, don\'t forget to bring Vim back to the foreground later!'
        end
