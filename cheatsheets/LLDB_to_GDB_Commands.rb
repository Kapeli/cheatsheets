cheatsheet do
	...
	category do
		...
		entry do
			...
			```
			(gdb) break main
			<i>(Hope that there are no C funtions named main)</i>.
		...
		entry do
			...
			```
			(gdb) break count
			<i>(Hope that there are no C or C++ funtions named count)</i>.
	...
	category do
		...
		entry do
			...
			(lldb) bt 5 (<i>lldb-169 and later</i>)
			(lldb) bt -c 5 (<i>lldb-168 and earlier</i>)
		...
		entry do
			...
			<i>NOTE: any command can inline a scalar expression result (as long as the target is stopped) using backticks around any expression:</i>
